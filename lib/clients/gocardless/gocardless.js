...
http.interceptors.response.use(
  ...
  error => {
    if (error.response) {
      ...
      return Promise.reject({ errors: error.response.data.error.errors });
...
const buildSubscription = subscription => {
  return {
    ...
    ..._pick(subscription, 'id', 'amount', 'currency', 'status'),
  };
};

export function deleteSubscription(id) {
  return http.post(`/subscriptions/${id}/actions/cancel`);
}
